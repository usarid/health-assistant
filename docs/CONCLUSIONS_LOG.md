...
| C-021 | confirmed | Stanford's LoadReportContent endpoint returns DIFFERENT payloads based on calling origin. Called from `mychart.stanfordhealthcare.org` (iframe origin, what our v3 scrape uses): returns a 755-char rendering skeleton with 0 chars of visible text. Called from `myhealth.stanfordhealthcare.org` (wrapper origin, used by Stanford's UI when the user clicks My Medical Record → Patient Visit Records → VIEW): returns the full clinical note HTML. Same endpoint path, same body, same credentials — only the Origin/Referer/Sec-Fetch-Site differ. UCSF doesn't do this (the same iframe-origin call returns full content there). Implications for v3: capturing Stanford's note bodies requires either (a) running the runtime in a wrapper-origin context with its own anti-forgery token initialization, or (b) automating the UI click-through (Patient Visit Records → select → VIEW per visit) with a fetch-hook capturing LoadReportContent responses. Tracked as task #19 | 2026-06-06 |
