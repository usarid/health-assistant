...
| C-019 | confirmed | C-017/C-018 recovery path validated end-to-end at Stanford: 106 clinical notes scraped via direct portal access (first time Stanford notes have ever been captured), 73% AHR-manifest date-coverage on first attempt. Three architectural findings while doing it: (a) Stanford embeds Epic via cross-origin iframe (`myhealth.* → mychart.*`); the actual Epic MyChart lives at `mychart.stanfordhealthcare.org/myhealth_sso/` after SSO. (b) Direct navigation to mychart.* triggers `body {display:none}` frame-busting that the runtime must override via CSS injection before any UI it injects can render. (c) AHR's numeric CSNs and MyChart's `WP-24`-encrypted CSNs are the same encounters keyed differently — cross-channel matching requires date+type tuples, not CSN | 2026-06-06 |
| C-021 | confirmed | Stanford's LoadReportContent endpoint returns DIFFERENT payloads based on calling origin: from `mychart.*` (iframe) → 755-char skeleton, 0 visible text; from `myhealth.*` (wrapper) → full clinical note HTML (~160-230 KB). UCSF doesn't discriminate this way. Confirmed live recipe (2026-06-06): user navigates VISITS → Past (AVS/Notes) tab → VIEW AVS/NOTES per visit → after-visit-summary page → "Clinical Notes" tab. Clicking that tab fires (via XHR, NOT fetch) `POST mychart.../myhealth_sso/api/visit-notes/GetVisitNotes?lang=en-US` (returns the note list, ~600 bytes per visit) then one `POST mychart.../myhealth_sso/api/report-content/LoadReportContent?lang=en-US` per note (returns full HTML body). Browser includes mychart cookies via cross-origin same-site with credentials. The wrapper's anti-forgery token lives in sessionStorage; we don't need to extract it — running the scraper from the wrapper page lets the page's own machinery handle the header. Bulk scrape recipe (task #19): for each past visit CSN, navigate to `https://myhealth.stanfordhealthcare.org/signedin/appointments/after-visit-summary/csn=<URL-encoded CSN>&encType=3`, install XHR hook, programmatically click `.tabs li:nth-child(2)` (the Clinical Notes tab), capture each LoadReportContent response | 2026-06-06 |
| C-022 | confirmed | Stanford fingerprints **injected keepalive fetches** as bot signature and revokes the session — even when those fetches are identical-on-the-wire to Stanford's own page-code keepalive pings (same URL, headers, cookies, both return HTTP 200). In the iOS Flutter prototype, adding a `fetch('keepalive.asp', {credentials:'include'})` to the per-visit injected JS caused session revocation after exactly 1 successful scrape. Removing both the per-visit JS keepalive and the Dart `Timer.periodic` keepalive let the batch run cleanly. Internal signal Stanford uses is likely caller context (script-injection vs page-native), call-stack origin, or some timing fingerprint. Practical consequence: don't add synthetic keepalive when the natural scrape cadence (~10s/visit) is already well below the session-timeout window (~10-15 min); each `loadUrl` is itself a session-extension event. Keepalive may still be wanted for portals that *require* it (UCSF? — untested) and for between-batch pauses, but should be opt-in per portal config, not default on | 2026-06-13 |
| C-020 | confirmed | MyChart's Visits page is outpatient-centric — inpatient documentation (daily progress notes, care plan notes, discharge summaries, H&P, consults) doesn't surface as individual visit rows even after full pagination. 28/28 Stanford AHR-only note dates had zero matching visit in v3's 142-visit MyChart roster; the type histogram (26 Progress + 15 Care Plan + Discharge Summary/Instructions + H&P + Consults + Procedures) is characteristic of hospital stays. v3 needs a second discovery surface (Hospitalizations / Documents page) to close inpatient gap. Not a scraper bug; an architectural blind spot in the Visits-only discovery model | 2026-06-06 |
