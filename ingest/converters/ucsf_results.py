...
SCRIPT_NAME = 'convert_ucsf_results_to_fhir.py'
SOURCE_FILE = 'ucsf_test_results_detail.json'
SOURCE_CODE = 'ucsf-mychart-results'
SOURCE_DISPLAY = 'UCSF MyChart Test Results Scrape'
...
def build_observation(component, order_name, effective_dt, provider, result_id_base):
    ...
    obs_id = make_id('ucsf-obs', comp_id or result_id_base, name, effective_dt or '', str(_obs_seq))
    ...
    observation['identifier'] = [{
        'system': 'urn:ucsf:mychart:component',
...
def build_diagnostic_report(order_data, observation_ids):
    ...
    report_id = make_id('ucsf-dr', order_key or order_name, effective_dt or '')
...
def main():
    with open('ucsf_test_results_detail.json') as f:
    ...
    for i, batch_entries in enumerate(batches):
        ...
        filename = f'ucsf_results_fhir_batch_{i+1}.json'
        with open(filename, 'w') as f:
            json.dump(batch_bundle, f, indent=2)
        print(f'  Batch {i+1}: {len(batch_entries)} resources -> {filename}')

    print(f'\nConverted {len(entries_with_data)} UCSF test results:')
