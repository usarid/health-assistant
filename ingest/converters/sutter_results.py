#!/usr/bin/env python3
"""Convert MSKCC MyChart test results (from GetDetails API) to FHIR R4 resources.
...
SCRIPT_NAME = 'convert_sutter_aff_results_to_fhir.py'
SOURCE_FILE = 'sutter_affiliates_test_results_detail.json'
SOURCE_CODE = 'sutter-affiliates-mychart-results'
SOURCE_DISPLAY = 'Sutter Affiliates MyChart Test Results Scrape'
...
def parse_abnormal_flag(flag_str):
    """Map MSKCC abnormal flag to FHIR interpretation code."""
    mapping = {
        'Low': ('L', 'Low'),
        'High': ('H', 'High'),
        'Abnormal': ('A', 'Abnormal'),
...
def build_observation(component, order_name, effective_dt, provider, result_id_base):
    ...
    obs_id = make_id('sutter-aff-obs', comp_id or result_id_base, name, effective_dt or '', str(_obs_seq))
    ...
    observation['identifier'] = [{
        'system': 'urn:sutter-affiliates:mychart:component',
...
def build_diagnostic_report(order_data, observation_ids):
    ...
    report_id = make_id('sutter-aff-dr', order_key or order_name, effective_dt or '')
    ...
    if order_key:
        report['identifier'] = [{
            'system': 'urn:sutter-affiliates:mychart:order',
            'value': order_key
        }]
...
def main():
    with open('sutter_affiliates_test_results_detail.json') as f:
    ...
    for entry in entries_with_data:
        ...
        # Also pull date from list entry if detail metadata is empty
        list_entry = entry.get('listEntry', {})
        list_meta = list_entry.get('orderMetadata', {})

        effective_dt = parse_iso_date(meta.get('prioritizedInstantISO')) or \
                       parse_iso_date(list_meta.get('prioritizedInstantISO')) or \
                       parse_iso_date(meta.get('latestUpdateInstantISO')) or \
                       parse_display_date(meta.get('resultTimestampDisplay'))

        provider = meta.get('authorizingProviderName') or meta.get('orderProviderName') or \
                   list_meta.get('authorizingProviderName') or list_meta.get('orderProviderName', '')

        # If detail has no order name, fall back to list entry
        if not order_data.get('orderName'):
            order_data['orderName'] = list_entry.get('name', 'Unknown Test')

        # Build observations for each component
        observation_ids = []
        order_key = order_data.get('key', '') or entry.get('key', '')
    ...
    for i, batch_entries in enumerate(batches):
        ...
        filename = f'sutter_aff_results_fhir_batch_{i+1}.json'
        with open(filename, 'w') as f:
            json.dump(batch_bundle, f, indent=2)
        print(f'  Batch {i+1}: {len(batch_entries)} resources -> {filename}')

    print(f'\nConverted {len(entries_with_data)} Sutter Affiliates test results:')
