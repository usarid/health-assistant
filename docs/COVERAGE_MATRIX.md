...
**Last counts taken:** 2026-08-10 post-Medications ingest (HAPI on localhost:8090).
**Coverage matrix version:** 7.
...
| MedicationRequest | mixed + Stanford native (fresh) | ✅ | ✅ | 759 | ✅ Meds tab | ✅ | **Phase 7 solved 2026-08-10.** Stanford Medications page embeds the full Rx list as a JS-object-literal argument to `MedicationRefillWorkflowController(...)` inside a 132 KB `<script>` block. Fetcher extracts via boundary-anchored script-block walk + `new Function()` eval-with-stub. 20 Stanford Rx now in HAPI (6 active refill-enabled, 14 completed refill-disabled, 10 flagged patient-reported). Auto-refreshed on every sign-in. |
...
- **2026-08-10 (evening)** — v7. Phase 7 reversed: previous "portal scrape dead-end" verdict was wrong. Stanford's `/Clinical/Medications` HTML embeds the full Rx list as a JS-object-literal argument to `MedicationRefillWorkflowController(...)` inside a 132 KB `<script>` block. Solved via boundary-anchored script-block walk + `new Function()` eval-with-stub (stub captures the arg as a side-effect; post-capture $$WP.Utilities.UI errors swallowed with inner try/catch). 20 Stanford Rx (6 active + 14 completed, 10 patient-reported) now in HAPI as `urn:stanford:myhealth:medication-rx`. Auto-refreshed on every sign-in. Deferred Epic SMART-on-FHIR task no longer critical — could still improve coverage across other resource types but not urgent.
