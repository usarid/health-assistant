...
SCRIPT_NAME = 'convert_mskcc_results_to_fhir.py'
SOURCE_FILE = 'mskcc_test_results_detail.json'
SOURCE_CODE = 'mskcc-mychart-results'
SOURCE_DISPLAY = 'MSKCC MyChart Test Results Scrape'
...
def build_observation(component, order_name, effective_dt, provider, result_id_base):
    ...
    obs_id = make_id('mskcc-obs', comp_id or result_id_base, name, effective_dt or '', str(_obs_seq))
    ...
    observation['identifier'] = [{
        'system': 'urn:mskcc:mychart:component',
...
def build_diagnostic_report(order_data, observation_ids):
    ...
    report_id = make_id('mskcc-dr', order_key or order_name, effective_dt or '')
    ...
    if order_key:
        report['identifier'] = [{
            'system': 'urn:mskcc:mychart:order',
            'value': order_key
        }]
...
def main():
    with open('mskcc_test_results_detail.json') as f:
    ...
    for i, batch_entries in enumerate(batches):
        ...
        filename = f'mskcc_results_fhir_batch_{i+1}.json'
        with open(filename, 'w') as f:
            json.dump(batch_bundle, f, indent=2)
        print(f'  Batch {i+1}: {len(batch_entries)} resources -> {filename}')

    print(f'\nConverted {len(entries_with_data)} MSKCC test results:')
